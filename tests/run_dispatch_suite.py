"""Run dispatch-model test suite against Coze bot chat API.
...
SUITE_PATH = Path(__file__).resolve().parent / "dispatch_suite.json"
REPORT_DIR = Path(__file__).resolve().parent / "reports" / "dispatch_model"
...
def _match_expected_param(key: str, expected: Any, params: Dict[str, Any]) -> bool:
    ...
    *,
    ...
    if isinstance(expected_params, dict) and expected_params:
        for k, v in expected_params.items():
            if not _match_expected_param(k, v, actual_params):
                params_ok = False
                failures.append(
    ...
    if expected_action:
        if not _match_expected_param("action", expected_action, actual_params):
            params_ok = False
            failures.append(
...
def main() -> int:
    ...
    if not isinstance(cases, list) or not cases:
        raise SystemExit("dispatch_suite.json 中的 用例 必须为非空数组")
