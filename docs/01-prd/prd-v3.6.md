# NewsFlow.ai 产品需求文档 v3.6

> 文档版本：v3.6
> 状态：体验优化已上线（浮窗引导、邮件品牌修复），持续迭代中
> 最后更新：2026-03-06
> 语言：中文
> 相关文档：`docs/prd等过程文件/04-模型选项分析报告/02-模型选型分析报告-v2.0.md`、`docs/prd等过程文件/05-Prompt工程手册/03-Prompt工程手册-v3.0.md`、`docs/prd等过程文件/06-评测与质量报告/04-评测与质量报告-v3.0.md`、`docs/seed_user_research_report.md`

---

## 0. 本文档定位

本 PRD 是在 MVP 交付后、种子用户测试完成后的一次全面梳理，目的是：

1. **对齐现状**：准确记录系统已实现的全部能力（含代码中已有但旧版 PRD 未覆盖的部分）
2. **沉淀反馈**：将种子用户测试的关键洞察写入 PRD，形成可执行的迭代优先级
3. **规划迭代**：基于测试反馈，明确下一步优化方向

与 v2.0 的区别：v2.0 侧重"已交付 + 面试展示"，v3.5 侧重"真实现状 + 种子测试结果 + 迭代规划"。
...
| 时间浪费 / 每天翻多个信源   | 省时     | 定时推送自动送达 + 即时查询秒级响应 + 热点排序               | 首字响应时间；日报阅读时长   | P50~4s（§3.1）；主流用户阅读时长 1-2 分钟，深度用户 5-10 分钟（§3.2） |
| 信任缺失 / AI 内容不可信    | 可信     | 每条新闻带原文链接 + 权威信源白名单 + 热度标注（≥2 家报道） | 用户主观信任度；链接可用率   | 用户反馈内容"中立和事实"（§3.2）；人工抽检链接可用率 100%（§3.1）     |
| 获取门槛高 / RSS 有技术门槛 | 零门槛   | 对话交互（说句话就能查）+ 邮箱订阅（填个邮箱就能收）         | 订阅转化率；用户主动使用频次 | 50%用户表示愿意推荐（NPS良好）；深度用户反馈每天节省10-15分钟（§3.2）  |

> 种子用户测试已完成（2026-03-04），核心痛点（信息焦虑、时间浪费）已获验证，用户反馈内容"中立和事实"，50%用户表示愿意推荐，NPS表现良好。详见 §3.2 用户价值指标。
...
种子测试（2026-03-04完成）共收集 8 位用户深度反馈，验证了产品核心价值假设。

#### 核心发现

| 指标             | 结果                                         | 目标               | 状态      |
| ---------------- | -------------------------------------------- | ------------------ | --------- |
| 核心价值验证     | 100% 用户认可"高信噪比"、"节省筛选时间"价值  | 定性验证通过       | ✅ 达标   |
| NPS（推荐意愿）  | 50% 用户明确表示会向同事/朋友推荐            | ≥30%              | ✅ 达标   |
| 阅读时长         | 主流 1-2 分钟（速览型），深度用户 5-10 分钟  | 有即可             | ✅ 达标   |
| 时间节省感知     | 深度用户反馈每天节省 10-15 分钟信息获取时间  | 有即可             | ✅ 达标   |
| 内容信任度       | 用户反馈内容"中立和事实"，优于公众号/短视频  | 定性验证通过       | ✅ 达标   |
| 邮件日报持续阅读 | 25% 每日必读，50% 2-3天查阅，25% 低频/未激活 | ≥50% 7 日后仍在看 | ⚠️ 观察 |
| 反馈正向率       | 待统计（埋点数据）                           | ≥70%              | ⏳ 统计   |

#### 关键洞察

**正面反馈**：

- 用户普遍认可产品"信息集中且有针对性"、"中立和事实"的核心价值
- 已形成部分习惯用户（通勤路上、工作前查阅）
- "方便高效，省去了收集信息的时间"

**关键摩擦点**：

1. **推送时间不匹配**：早 8 点易被工作邮件淹没，用户期望 9:00-10:00 自选
2. **发件人缺乏信任感**：私人头像不够"官方"，需要品牌 Logo
3. **取消订阅入口隐蔽**：Bot 中操作不够直观
4. **测试引导不清晰**：用户期望订阅前预览邮件效果

#### 待验证假设（P2阶段）

- **付费意愿**：产品经理用户提出的"个性化摘要生成"被视为潜在付费功能（Aha Moment）
- **多主题需求**：宽领域用户期望同时订阅多个主题

> 详细调研报告见：`docs/seed_user_research_report.md`
...
分散在各功能章节的非功能需求汇总如下，方便快速查阅整体约束：

| 维度 | 指标/要求 | 目标值 | 当前值 | 验证方式 | 详细位置 |
|------|-----------|--------|--------|----------|----------|
| **性能** | 首字响应 P50 | ≤5s | ~4s | 自动化评测 | §3.1 |
| **性能** | 首字响应 P90 | ≤90s | ~66s | 自动化评测 | §3.1 |
| **性能** | 即时查询响应 | 快速、低延迟 | 科技类为主 | 人工抽检 | §5.1、§6.1 |
| **稳定性** | 单轮通过率 | ≥95% | 100% | L3 端到端测试 | §3.1 |
| **稳定性** | 多轮通过率 | ≥85% | 94.12% | L3 端到端测试 | §3.1 |
| **稳定性** | 工作流执行成功率 | ≥90% | 96.3% | L3 端到端测试 | §3.1 |
| **质量** | 端到端压缩比（去噪） | 有即可 | 125:1（即时）、16:1（定时） | 数据测试 | §3.1 |
| **质量** | 链接可用率 | 100% | 100% | 人工抽检 | §3.1 |
| **安全** | Prompt 注入防御 | 实施 | 已实施 | 代码审查 | §11.2 |
| **安全** | 用户隔离 | 实施 | 已实施（user_id 强绑定） | 代码审查 | §11.2 |
| **安全** | 接口限流 | 30-60/min | 已实施 | 代码审查 | §2.1、§11.2 |
| **安全** | 密钥管理 | 环境变量 | 已实施 | 代码审查 | §11.2 |
| **运维** | 健康检查 | 实施 | `/health` 端点 | 部署验证 | §2.1 |
| **运维** | 埋点监控 | 实施 | 前端+后端事件 | 线上观测 | §2.1、§3.3 |
...
**体验优化（2026-03-06上线）**：
- ✅ **浮窗引导增强**：PC端首次访问自动展示引导气泡（5秒）+ hover触发；移动端增加"✨ 试试AI助手"浮动标签
- ✅ **邮件品牌修复**：发件人名称从邮箱地址改为"NewsFlow.ai 每日简报"，配合专属Logo提升信任感
- ✅ **Bug修复**：提示词限定24h查询范围、邮件标点截断问题修复、即时查询排序提示词优化

### 12.2 种子测试阶段（已完成，2026-03-04）

目标：找 5-10 个种子用户测试，验证产品价值。

目标达成情况：

- ✅ AI 主题测试：完成，8 位用户深度反馈（5 位 AI 主题 + 3 位财经主题）
- ⚠️ 财经主题测试：部分完成（样本偏少，需后续补充）
- ✅ 用户反馈收集：完成，核心结论已沉淀至 §3.2
- ✅ 运营指标观察：完成
- ⏳ 单用户成本粗算：待补充（依赖后续账单统计）

**关键结论**：100% 用户认可"高信噪比/节省筛选时间"；50% 表示愿意推荐；主要摩擦为推送时间错位、信任背书不足、取消订阅入口隐蔽。详见 §3.2 用户价值指标与 `docs/seed_user_research_report.md`

### 12.3 下一步规划

基于种子测试反馈和已上线的体验优化，聚焦**内容价值提升**和**交互优化**。

#### P0 - 内容价值提升（快速见效）

| 任务 | 来源 | 实现方案 | 预期收益 |
|------|------|---------|---------|
| **简报摘要增加要点和影响** | 产品思考 | 在现有摘要基础上，增加"核心要点"和"影响解读"段落（1-2句话） | 提升信息价值密度，差异化于纯资讯聚合 |
| **即时查询补充财经信源** | 体验短板 | 增加快速响应的财经专业信源（如华尔街见闻、财新等） | 解决财经查询结果少、质量不稳定问题 |
| **订阅确认邮件** | 体验优化 | 用户订阅成功后立即发送一封确认邮件（含欢迎语 + 下次推送时间） | 增强仪式感，确认邮箱有效性 |

#### P1 - 交互优化（基于数据观测）

| 任务 | 来源 | 实现方案 | 预期收益 |
|------|------|---------|---------|
| **取消订阅显性化** | 用户反馈 | 首页"立即订阅"按钮增加状态切换（已订阅 → 取消订阅），或在订阅成功页增加管理入口 | 完成订阅生命周期闭环 |
| **测试邮件入口优化** | 用户反馈 | 将测试邮件功能从浮窗弱化，改为订阅表单下方文案链接："犹豫要不要订阅？先发送测试邮件看看效果" | 降低订阅决策门槛，减少浮窗干扰 |
| **个性化推送时间** | 用户1,8 | `subscriptions` 表新增 `send_time` 字段（默认 10:00，支持 8:00/9:00/10:00 三档），n8n 调度器按时间段分组触发 | 解决"被工作邮件淹没"问题，提升打开率 |

#### P2 - 探索性需求（长期价值）

| 任务 | 来源 | 实现方案 | 预期收益 |
|------|------|---------|---------|
| **个性化 Agent 摘要（Aha Moment）** | 用户8 | 引入用户画像标签系统：允许用户预设关注标签（公司/技术/行业）→ 大模型在日报顶部生成"基于个人职业视角"的个性化总结 | 被用户视为"愿意付费"的功能 |
| **多主题订阅** | 用户8 | 支持同时订阅多个主题，后端做内容融合去重 | 满足宽领域用户需求 |
| **一周简报** | 产品思考 | 在日报基础上，每周五发送"本周精选"，汇总本周最重要3-5条新闻及趋势解读 | 增加内容价值层次，提升用户粘性 |
| **时间范围扩展** | 产品思考 | 支持查询3天内/一周内/一个月内的新闻（需RAG知识库支撑） | 满足深度研究需求，但工程量大，v2.0再考虑 |
| **API信源（X/Reddit）** | 产品思考 | 增加海外社交平台信源（需评估API成本和内容质量） | 扩展信息源，但中文用户价值待验证，暂缓 |

#### 技术债务（与功能迭代并行）

| 任务              | 说明                                   | 优先级                  |
| ----------------- | -------------------------------------- | ----------------------- |
| 豆包 1.6 替代方案 | 关键词扩写模型切换，保证响应速度 ≤ 2s | 高（模型即将下架）      |
| 即时查询信源补充  | 补充财经和 AI 专业信源                 | 中                      |
| 延迟优化          | 目标：平均 < 5s，P99 < 10-12s          | 中                      |
| 定时推送送达监控  | 记录 SMTP 返回状态，统计送达率         | 低（用户量 >50 时优先） |

> 种子用户详细反馈见：`docs/seed_user_research_report.md`
...
| 文档                  | 路径                                                                   |
| --------------------- | ---------------------------------------------------------------------- |
| PRD v3.6（本文档）    | `docs/prd等过程文件/01-prd/prd-v3.6.md`                              |
| PRD v3.5              | `docs/prd等过程文件/01-prd/prd-v3.5.md`                              |
...
| 时间 | 问题 | 修复方案 | 状态 |
|------|------|---------|------|
| 2026-03-06 | 提示词未限定查询时间范围（24h） | 在 System Prompt 中明确增加"仅输出最近 24 小时内内容"的约束 | ✅ 已修复 |
| 2026-03-06 | 推送邮件标点符号截断问题 | 优化邮件标题/正文的截断逻辑，避免在标点处截断 | ✅ 已修复 |
| 2026-03-06 | 即时查询排序稳定性不足 | 二次优化提示词，强化"按报道数量→跨媒体类型数→发布时间"的排序规则 | ⚠️ 提示词已优化，待进一步测试验证 |
| 待处理 | 对话交互：新消息在底部而非置顶 | 需要调整浮窗对话的滚动策略（当前符合常规聊天习惯，待确认是否为真实痛点） | ⏳ 待评估 |
