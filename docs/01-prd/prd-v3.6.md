...
| 能力     | 说明                                                                                                                         |
| -------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 埋点系统 | 前端事件（page_view、client_error、chat_trigger_click、chat_guide_click）+ 后端事件（workflow_call、email_sent、subscribe、feedback、email_open、email_link_click），写入 Supabase `events` 表 |
...
| `email_tracking`     | 邮件追踪表（打开、点击统计）                                            | 在用               |
...
累计记录 **25 个 Bad Case**，分两个阶段（全部已解决）：
...
| 邮件推送 | 2      | 0      | 科技被 AI 淹没、摘要超标、微信 QQ 邮箱摘要乱码            |
| 评测工具 | 1      | 0      | 评测器解析问题                                            |
| 工作流   | 0      | 0      | —                                                         |

> 2026-03-13 更新：原 3 个观察中问题（`MP-TECH-01` 科技主题 AI 占比、`MP-SUMMARY-LEN` 摘要字数、`E041` 工作流异常）已全部修复关闭。新增 `BC-PUSH-004` 微信 QQ 邮箱摘要乱码问题已修复。
