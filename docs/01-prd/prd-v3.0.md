...
NewsFlow.ai 是一个 AI 驱动的新闻聚合与订阅推送助手，通过对话交互串联"查新闻、订新闻、收新闻"三条核心链路，帮助用户以最低的时间成本获取每日行业资讯。
...
- 不一定是从业者，但有持续获取资讯的习惯或意愿
- 典型画像：AI 产品经理、对 AI 感兴趣的职场人、科技爱好者
...
**主要痛点（已验证）**：
1. 信息过载：每天要翻多个信源，重复内容多、噪音大
2. 时间焦虑（FOMO）：担心漏掉重要事件，又没时间逐条看

**次要痛点（待验证）**：
3. 信任断层：AI 摘要可能存在幻觉 / 时间错乱
4. 工具断层：RSS 维护成本高，推荐流容易信息茧房
...
来自 v1.0 核心回归集（22 条），作为后续迭代的参照基线：
...
| 指标 | 测试方法 | 优先级 |
| --- | --- | --- |
| 死链率 | 脚本抓取 AI 输出的所有链接，HTTP HEAD 检查状态码，统计 404 / 超时比例 | 中（种子测试前建议跑一次） |
| 去噪比 | RSS 原始条数 → AI 最终输出条数，计算压缩比 | 低（内部参考） |
...
| 邮件发送成功率 | `daily_email_logs` 表 | 确保推送链路稳定 |
...
| 日报生成 | 覆盖面广、内容质量高 | 16 个（去重） | 按主题分三个节点（科技/财经/AI&科技），覆盖较全面 |
...
graph TD
    ...
    subgraph "后端服务"
        ...
        API_Sub["/api/subscription<br/>(订阅 API)"]
        API_FB["/api/feedback<br/>(反馈 API)"]
        API_Track["/api/track<br/>(埋点 API)"]
        API_Chat["/api/chat<br/>(对话代理)"]
    end
    ...
    subgraph "业务工作流（在用）"
        ...
        W3["send_mail_cache_using<br/>(测试推送/缓存)"]
...
sequenceDiagram
    ...
    Note left of U: 路径B-2：AI 浮窗订阅管理
    U->>CW: "帮我改成财经主题"
    ...
    Note left of U: 路径C：定时推送（系统自动）
    Note over WF: 每日 08:00
    WF->>WF: supabase_auto_gen_news: 抓取 RSS
    WF->>WF: DeepSeek V3.2 生成 HTML
    WF->>DB: 写入 daily_news_history（生成一次）
    WF->>DB: supabase_auto_send_only_mail: 查询活跃订阅
    WF->>DB: 读取 daily_news_history 缓存
    WF->>Mail: 批量发送（分发多次）
    end

    rect rgb(255, 250, 240)
    Note left of U: 反馈与埋点（不经过 Agent）
...
本项目在 Coze 平台可选模型范围内采用"轻重分层"策略：

| 任务类型 | 模型 | 选择理由 | 使用场景 |
| --- | --- | --- | --- |
| 轻量任务 | 豆包 1.6 极致速度 | 响应快、成本低，适合确定性高的任务 | 关键词扩写、意图路由辅助 |
| 重量任务 | DeepSeek V3.2 | 理解力强、生成质量高 | 新闻聚类排序、深度摘要、HTML 日报生成 |
...
用户说"大模型新闻"，Agent 传 `keyword=ai`（归一化），但 raw_query 保留原话。这样下游模型既能走正确的信源分支，又能理解用户的具体意图。
...
| 评测集 | 规模 | 用途 |
| --- | --- | --- |
| 核心回归集 | 22 条 | 保证关键路径闭环（订阅、查询、推送） |
| 扩展评测集 | ~100 条 | 覆盖一级/二级意图、多轮对话、边界输入 |
...
### 10.3 Bad Case 汇总（v2.0 原有 + 新增）

#### 已解决（v2.0 原有 8 例）

| ID | 问题 | 根因 | 修复 |
| --- | --- | --- | --- |
| Bug-03 | 时间幻觉（发布时间不符/时区错乱） | 信源时间格式不统一 + 未做时区转换 | 代码节点标准化时间（强制北京时间） |
| Bug-05 | 响应超时（>40s） | 原始列表直接喂模型，token 过大 | 代码节点预筛选 Top-K + 流式输出 + 模型升级 |
| Bug-06 | 多轮关键词未保存 | 关键词提取规则薄弱 | Prompt 增加提取规则与 few-shot |
| Bug-07 | 多轮邮箱补充异常 | 上下文槽位复用策略不稳定 | Prompt 增加"已给出则复用"规则 |
| Bug-11 | 邮件纯文本视觉差 + token 消耗大 | 未使用结构化模板 | 引入 HTML 模板 + Query 改写/预筛选 |
| Bug-12 | System Prompt 泄露 | 缺少对抗注入规则 | Prompt 头部加入强边界 + 关键字触发拒答 |
| Bug-13 | 类别幻觉（编造不存在的类别） | 未限制可选主题范围 | Prompt 明确支持范围 + 兜底话术 |
| Bug-16 | 隐私越权（可查他人配置） | 缺少强边界 + 存储侧未强绑定 | Prompt 硬约束 + 调用侧强制 user_id |

#### 新增已解决（来自最新 badcase 表）

| ID | 问题 | 根因 | 修复 |
| --- | --- | --- | --- |
| U-04 | 小写主题归一化失败 | keyword 大小写处理不一致 | 归一化逻辑修复 |
| U-NEG-02 | 不支持的主题未正确提示 | 缺少兜底提示 | 增加不支持主题的友好提示 |
| D-03 | 取消后重新订阅异常 | 状态流转逻辑缺陷 | 修复订阅状态机 |
| GEN-05 | 来源过滤失效 | 模型未严格执行来源约束 | Prompt 增加来源过滤自检机制 |
| NEWS-T06/T10/T13/T14 | 来源过滤/来源约束不稳定 | 多个来源相关的边界 case | Prompt 增加白名单 + 别名映射 + 逐条自检 |
| NEWS-T18/T20 | 来源字段/格式约束 | 输出格式不统一 | Prompt 增加排序键可解析约束 |
| NEWS-T03 | 输出结构模板不稳定 | 缺少强制模板 | Prompt 增加结构化输出模板 |
| NEWS-T04 | AI 精确来源（虎嗅）过滤失败 | 来源匹配不精确 | 来源过滤自检 + 精确匹配规则 |
| NEWS-T05 | AI 别名来源（36kr）识别失败 | 缺少别名映射 | 增加别名映射表 |
| DISPATCH-EVAL-REPLY-01 | 评测器回复提取问题 | 评测工具解析逻辑 | 修复评测器 |
| DISPATCH-PARAM-01 | 参数提取实体关键词归类错误 | Agent 层过度归类 | 改为实体透传策略 |
| DISPATCH-ROUTE-01 | 意图路由误判 | 意图识别表不够精确 | 优化意图识别表 + 优先级排序 |
| DISPATCH-MISCALL-01 | 缺失必填参数未追问直接调用 | 缺少调用前检查 | 引入调用守卫（Calling Guard） |

#### 持续观察中

| ID | 问题 | 状态 | 说明 |
| --- | --- | --- | --- |
| MP-TECH-01 | 科技主题被 AI 新闻淹没 | 部分改善 | 与信源分布有关，科技源中 AI 内容占比高 |
| MP-SUMMARY-LEN | 摘要字数严重超标 | 观察中 | 模型有时忽略字数限制 |
| E041 | 工作流执行异常 | 待观察 | 偶发性问题 |
...
| 文档 | 路径 |
| --- | --- |
| PRD v2.0（完整版） | `docs/prd等过程文件/01-PRD-产品需求文档-v2.0-完整版.md` |
| PRD v2.0（精简版） | `docs/prd等过程文件/01-PRD-产品需求文档-v2.0.md` |
| 模型选型分析报告 | `docs/prd等过程文件/02-模型选型分析报告.md` |
| Prompt 工程手册 | `docs/prd等过程文件/03-Prompt工程手册.md` |
| 评测与质量报告 | `docs/prd等过程文件/04-评测与质量报告.md` |
| 项目复盘总结 | `docs/prd等过程文件/05-项目复盘总结.md` |
| 信源管理 | `docs/信源管理.md` |
| 埋点说明 | `docs/埋点说明.md` |
